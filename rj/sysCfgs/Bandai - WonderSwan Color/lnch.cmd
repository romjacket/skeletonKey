...
for %%A in ("C:\Emulators\mednafen") do SET EMUL=%%~A
for %%A in ("mednafen.exe") do SET EMUZ=%%~nA
...
for /f "delims=" %%a in ('dir /B /A-D "*.bin" "*.zip" "*.ws" "*.wsc"') do (
set ROMF=%%~a
set ROM=%%~na
set ROMX=%%~xa
set ROMFN=%%~nxa
...
%LAUNCH% "%EMUL%\%EMUZ%.exe" -fs 1 "%GAMDIR%\%ROMF%"
%CCMDY%
%RLOC%popd
%XFG%copy /y "%EMUL%\mednafen*.cfg" "%GAMDIR%"
