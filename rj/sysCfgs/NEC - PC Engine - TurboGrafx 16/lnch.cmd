...
for %%A in ("retroarch.exe") do SET EMUZ=%%~nA
...
for /f "delims=" %%a in ('dir /B /A-D "*.pce" "*.zip"') do (
set ROMF=%%~a
set ROM=%%~na
set ROMX=%%~xa
set ROMFN=%%~nxa
...
%LAUNCH% "%EMUL%\%EMUZ%.exe" -L cores\mednafen_pce_fast_libretro.dll "%GAMDIR%\%ROMF%"
%CCMDY%
%RLOC%popd
%XFG%retroArch
