...
for %%A in ("C:\Emulators\Mupen64plus") do SET EMUL=%%~A
for %%A in ("mupen64plus.exe") do SET EMUZ=%%~nA
...
[INIVARS]
EMUL="C:\Emulators\Mupen64plus"
EMUZ="mupen64plus.exe"
GAMNAM="[GAMNAM]"
...
for /f "delims=" %%a in ('dir /B /A-D "*.n64" "*.d64" "*.z64" "*.zip" "*.usa" "*.pal"') do (
set ROMF=%%~a
set ROM=%%~na
set ROMX=%%~xa
set ROMFN=%%~nxa
...
%LAUNCH% "%EMUL%\%EMUZ%.exe"mupen64plus"%GAMDIR%\%ROMF%"
%CCMDY%
%RLOC%popd
%XFG%copy /y "%EMUL%\*.ini" "%GAMDIR%"
%XFG%copy /y "%EMUL%\*.cfg" "%GAMDIR%"
